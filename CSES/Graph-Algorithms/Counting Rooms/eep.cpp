...
const vi64 x = {1, -1, 0, 0};
const vi64 y = {0, 0, 1, -1};
i64 lin, col;

void bfs(p64 start, vec<vec<i64>> &visit, vec<vec<i64>> &b, i64 c) {
  deque<p64> q;
  q.push_back(start);
  visit[start.first][start.second] = c;
  ...
  while (!q.empty()) {
    ...
    for (i64 i = 0; i < 4; i++) {
      i64 xx = crt.first + x[i];
      i64 yy = crt.second + y[i];

      if (xx >= 0 and yy >= 0 and xx < lin and yy < col and
          visit[xx][yy] == 0 and b[xx][yy] == 1) {
        visit[xx][yy] = c;
        q.push_back({xx, yy});
      }
    }
