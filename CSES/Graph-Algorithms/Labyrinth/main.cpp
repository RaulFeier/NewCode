...
int main() {
  ...
  vector<vector<char>> beep(n, vector<char>(m));
  ...
  for (i64 i : range(n)) {
    for (i64 j : range(m)) {
      if (beep[i][j] == '.' or beep[i][j] == 'A' or beep[i][j] == 'B') {
        if (i > 0) {
          if (beep[i - 1][j] == '.' or beep[i - 1][j] == 'A' or
              beep[i - 1][j] == 'B') {
            eep[n * i + j].push_back(n * (i - 1) + j);
          }
        }
        if (i < n - 1) {
          if (beep[i + 1][j] == '.' or beep[i + 1][j] == 'A' or
        ...
        if (j > 0) {
          if (beep[i][j - 1] == '.' or beep[i][j - 1] == 'A' or
              beep[i][j - 1] == 'B') {
            eep[m * i + j].push_back(m * i + j - 1);
          }
        }
        if (j < m - 1) {
          if (beep[i][j + 1] == '.' or beep[i][j + 1] == 'A' or
