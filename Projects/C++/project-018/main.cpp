...
*path[0..pi-1]: The path traversed by robot till now (Array to hold the
                                path need to have space for at least m+n
elements) */
void printAllPathsUtil(int *mat, int i, int j, int n, int *path, int pi) {
  ...
  printAllPathsUtil(mat, i + 1, j, n, path, pi + 1);

  // Print all the paths that are possible after moving right
  printAllPathsUtil(mat, i, j + 1, n, path, pi + 1);
...
void printAllPaths(int *mat, int n) {
  int *path = new int[n + n];
  printAllPathsUtil(mat, 0, 0, n, path, 0);
}
