...
char solve(char a, char b) {
  if (a == b) {
    return a;
  } else if ((a == 'R' and b == 'S') or (a == 'S' and b == 'R')) {
    return 'R';
  } else if ((a == 'S' and b == 'P') or (a == 'P' and b == 'S')) {
    return 'S';
  } else if ((a == 'P' and b == 'R') or (a == 'R' and b == 'P')) {
    return 'P';
  }
}
...
int main() {
  ...
  while (tc--) {
    ...
    for (i64 xx = 0; xx < 2; xx++) {
      string eep = "";
      for (i64 i = 0; i < ans.size(); i += 2) {
        eep += solve(ans[i], ans[i + 1]);
      }
      ans = eep;
    }
