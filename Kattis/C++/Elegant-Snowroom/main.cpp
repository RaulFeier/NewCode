...
i64 bfs(vector<vector<char>> &room, vv &dist, p64 start) {
  deque<p64> q;
  q.emplace_back(start);
  vec<vb> visit(room.size(), vb(room.front().size(), false));
  ...
  while (!q.empty()) {
    ...
    if (room[crt.first][crt.second] == 'D' and
        (crt.first == 0 or crt.first == room.size() - 1 or
         crt.second == room[0].size() - 1 or crt.second == 0)) {
    ...
    for (i64 i : range(4)) {
      ...
      if (xx >= 0 and xx < room.size() and yy >= 0 and
          yy < room.front().size() and room[xx][yy] != '#' and !visit[xx][yy]) {
        visit[xx][yy] = true;
        if (room[xx][yy] == 'D') {
          q.emplace_front(xx, yy);
          dist[xx][yy] = dist[crt.first][crt.second];
        } else {
...
int main() {
  ...
  vv dist(n, vector<i64>(m, INF));
  dist[start.first][start.second] = 1;

  cout << bfs(room, dist, start) << endl;
