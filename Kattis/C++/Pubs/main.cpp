...
void bipartite(i64 start, vec<vec<i64>> &adj, vi64 &color, vb &visit) {
  queue<i64> q;
  q.push(start);
  color[start] = 0;
  if (adj[start].empty()) {
...
void solve(vector<vector<i64>> &adj) {
  vi64 color(adj.size(), -1);
  vb visit(adj.size(), false);

  for (i64 i = 1; i < adj.size(); i++) {
    if (!visit[i]) {
      bipartite(i, adj, color, visit);
    }
  }

  if (bip) {
    for (i64 i = 1; i < color.size(); i++) {
      if (color[i] == 0) {
        cout << "pub ";
      } else {
        cout << "house ";
