...
d64 distance(d64 x1, d64 y1, d64 x2, d64 y2) {
...
public:
  ...
  bool IsSameSet(i64 x, i64 y) { return FindSet(x) == FindSet(y); }
...
int main() {
  ...
  while (tc--) {
    ...
    vec<pair<d64, d64>> v(n);

    for (auto &c : v) {
      cin >> c.ft >> c.sd;
    }

    vector<pair<d64, p64>> islands;
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        d64 dist = distance(v[i].first, v[i].second, v[j].first, v[j].second);
        islands.push_back({dist, {i, j}});
      }
    }
