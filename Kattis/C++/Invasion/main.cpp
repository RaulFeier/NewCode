...
vector<i64> dijkstra(i64 s, const vec<vp64> &adj, vi64 &dist) {
  dist[s] = 0;
  ...
  while (!pq.empty()) {
    ...
    if (dist[crt.sd] < crt.ft) {
      continue;
    }

    for (auto &c : adj[crt.sd]) {
      i64 cand = crt.ft + c.sd;
      if (dist[c.ft] > cand) {
        dist[c.ft] = cand;
        pq.emplace(cand, c.ft);
      }
    }
  }
...
int main() {
  ...
  while (true) {
    i64 n, m, alien_base, k;
    cin >> n >> m >> alien_base >> k;

    if (n == 0 and m == 0 and alien_base == 0 and k == 0)
      break;
