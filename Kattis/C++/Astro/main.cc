...
i64 convtime(str &eep) {
  p64 res = {0, 0};

  if (eep[0] != '0') {
    res.first = res.first * 10 + (eep[0] - '0');
  }
  res.first = res.first * 10 + (eep[1] - '0');

  if (eep[3] != '0') {
    res.second = res.second * 10 + (eep[3] - '0');
  }
  res.second = res.second * 10 + (eep[4] - '0');

  return res.first * 60 + res.second;
}
...
int main() {
  ...
  for (i64 i = s1; i < v.size(); i += t1) {
    v[i] = true;
  }

  bool check = false;
  for (i64 i = s2; i < v.size(); i += t2) {
    if (v[i]) {
      int day = i / 24 / 60 % 7;
      cout << days[day] << endl;
