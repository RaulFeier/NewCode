...
int main() {
  ...
  bool check = false;
  if (terms[terms.size() - 2] == 1e6) {
    check = true;
  }
  for (i64 i = 0; i < terms.size(); i++) {
    if (terms[i] == 1e6) {
      cout << '-';
    } else {
      cout << terms[i];
    ...
    if (check && i == terms.size() - 4) {
      cout << " and ";
    } else if (!check && i == terms.size() - 2) {
      cout << " and ";
    } else if ((terms[i + 1] != 1e6 && terms[i - 1] != 1e9 && terms[i] != 1e6 &&
                i < terms.size() - 4 && check) ||
               (terms[i + 1] != 1e6 && terms[i - 1] != 1e9 && terms[i] != 1e6 &&
                i < terms.size() - 2 && !check)) {
      cout << ", ";
    }
  }
  cout << endl;
  ...
  i64 aa = 0;
  for (i64 i = 0; i < n; i++) {
    crt.erase(crt.begin() + v[i] - 1 + aa);
    aa++;
  }

  for (i64 i = 1; i <= crt.size(); i++) {
    cout << crt[i] << " ";
  }
