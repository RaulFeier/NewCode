...
vi64 dij(i64 s, const vec<vec<pair<i64, Time>>> &g) {
  vi64 ans(g.size(), 1e18);
  ...
  while (!q.empty()) {
    ...
    if (dist == ans[node]) {
      for (auto &p : g[node]) {
        i64 dst = 1e18;
        if (p.second.t >= dist and p.second.p == 0) {
          dst = p.second.t + p.second.d;
        } else if (p.second.p > 0) {
          dst =
              p.second.t + max(0L, dist - p.second.t) / p.second.p * p.second.p;
          if (dst < dist)
            dst += p.second.p;
          dst += p.second.d;
        }
...
int main() {
  ...
  while (true) {
    ...
    vec<vec<pair<i64, Time>>> g(n);

    for (i64 i : range(m)) {
      i64 u, v;
      Time eep;
      cin >> u >> v >> eep.t >> eep.p >> eep.d;
