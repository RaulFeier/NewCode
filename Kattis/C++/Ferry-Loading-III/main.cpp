...
int main() {
  ...
  while (tc--) {
    ...
    for (i64 i : range(m)) {
      ...
      cars.push_back({nr, eep});
    }

    i64 total_time = 0;
    string initial_bank = "left";
    while (!cars.empty()) {
      if (total_time < cars.front().first) {
        total_time = cars.front().first;

        if (cars.front().second != initial_bank) {
          total_time += t;
          if (initial_bank == "right") {
            initial_bank = "left";
          } else {
            initial_bank = "right";
          }
