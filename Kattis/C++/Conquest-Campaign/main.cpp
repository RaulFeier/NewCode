...
vector<i64> dirx = {1, -1, 0, 0};
vector<i64> diry = {0, 0, -1, 1};

i64 bfs(vector<vector<i64>> &v, deque<p64> &q, vector<vector<bool>> &visit) {
  i64 res = 0;
  ...
  while (!dq.empty()) {
    ...
    while (!q.empty()) {
      ...
      for (i64 i : range(4)) {
        ...
        if (xx < v.size() and xx >= 0 and yy < v.front().size() and yy >= 0 and
            !visit[xx][yy]) {
          dq.push_back({xx, yy});
          visit[xx][yy] = true;
        }
...
int main() {
  ...
  vector<vector<i64>> v(r, vector<i64>(c, 0));
