...
int main() {
  ...
  map<str, vec<pair<string, i64>>> adj;
  map<string, obj> dist;
  vs v(n);

  for (i64 i : range(n)) {
    cin >> v[i];
    dist[v[i]] = {INF, INF, v[i]};
  }
  ...
  while (!q.empty()) {
    ...
    for (auto &c : adj[crt]) {
      if (dist[c.first].s > dist[crt].s + 1) {
        dist[c.first] = {dist[crt].s + 1, dist[crt].w + c.second, crt};
        q.push(c.first);
      } else if (dist[c.first].s == dist[crt].s + 1 and
                 dist[c.first].w >= dist[crt].w + c.second) {
        dist[c.first] = {dist[crt].s + 1, dist[crt].w + c.second, crt};
        q.push(c.first);
      }
    }
