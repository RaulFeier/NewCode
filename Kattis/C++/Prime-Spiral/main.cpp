...
void build(map<p64, i64> &num, vp64 &dist) {
  i64 currx = 0;
  i64 curry = 0;
  ...
  while (curr <= 10000) {
    for (i64 i = 0; i < 4 and curr <= 10000; i++) {
      for (i64 j = 0; j < steps and curr <= 10000; j++) {
        if (i == 0) {
          curry--;
        }
        if (i == 1) {
        ...
        if (!prime(curr)) {
          num[{currx, curry}] = curr;
          dist[curr] = {currx, curry};
        }
...
int main() {
  ...
  while (cin >> x >> y) {
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < 4; i++) {
        int xx = curr.first + dirx[i];
        int yy = curr.second + diry[i];
        if (!num.count({xx, yy})) {
          continue;
        }
        if (vis.count({xx, yy})) {
