...
d64 asct(vector<pair<d64, d64>> &asc, d64 time) {
  d64 completed_time = 0;
  d64 distance = 0;

  for (auto &x : asc) {
    if (completed_time < time and (x.second + completed_time) < time) {
      distance += x.first;
      completed_time += x.second;
    } else {
      distance += (x.first / x.second * (time - completed_time));
      return distance;
    }
  }
...
d64 desct(vector<pair<d64, d64>> &desc, d64 time, i64 total_distance) {
  d64 completed_time = 0;
  d64 distance = 0;
...
int main() {
  ...
  i64 a, d;
  cin >> a >> d;
  vector<pair<d64, d64>> asc;
  vector<pair<d64, d64>> desc;
  ...
  while (mid != (max + min) / 2) {
    mid = (max + min) / 2;
    d64 asc_distance = asct(asc, mid);
    d64 desc_distance = desct(desc, mid, total_distance);
