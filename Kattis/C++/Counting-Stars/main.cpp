...
void bfs(vector<vector<bool>> &visit, vector<vector<char>> &eep, i64 i, i64 j) {
  deque<p64> q;
  q.push_back({i, j});
  visit[i][j] = true;
  ...
  while (!q.empty()) {
    ...
    for (i64 i : range(4)) {
      ...
      if (xx >= 0 and yy >= 0 and xx < eep.size() and
          yy < eep.front().size() and !visit[xx][yy] and eep[xx][yy] == '-') {
        q.push_back({xx, yy});
        visit[xx][yy] = true;
      }
    }
