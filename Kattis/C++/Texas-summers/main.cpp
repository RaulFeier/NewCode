...
vp64 dij(i64 s, vec<vec<p64>> &g) {
  vp64 sol(g.size(), {INF, -1});
  pq<p64, vp64, greater<p64>> q;
  ...
  while (!q.empty()) {
    ...
    for (auto c : g[0]) {
      i64 x = c.first;
      i64 y = c.second;
      i64 d = (x - p.first) * (x - p.first) + (y - p.second) * (y - p.second);
      i64 val = dist + d;

      if (val < sol[index].first) {
        sol[index] = {val, node};
        q.push({val, index});
      }
      index++;
...
int main() {
  ...
  vec<vec<p64>> graph(n + 2);
  graph[0].push_back(start);

  for (auto &c : g) {
    graph[0].push_back({c.first, c.second});
  }

  graph[0].push_back(second);
  i64 s = 0;

  vp64 sol = dij(s, graph);

  i64 len = sol.size() - 1;
  di64 path;

  while (sol[len].second > 0) {
    path.push_front(sol[len].second - 1);
    len = sol[len].second;
  }
