...
void bfs(i64 i, i64 j, i64 r, i64 c, vec<vec<char>> &matrix,
         vec<vec<bool>> &vs) {
  deque<p64> d;
  d.push_back({i, j});
  vs[i][j] = true;
  while (!d.empty()) {
    ...
    d.pop_front();
    for (i64 i = 0; i < 4; i++) {
      int dx = dirx[i] + crt.second;
      int dy = diry[i] + crt.first;
      // in bounds not vised C && L
      if (dx > -1 && dx < c && dy > -1 && dy < r && vs[dy][dx] == false &&
          (matrix[dy][dx] == 'L' || matrix[dy][dx] == 'C')) {
        d.push_back({dy, dx});
        vs[dy][dx] = true;
      }
    }
...
int main() {
  ...
  vec<vec<char>> matrix(r, vec<char>(c));

  vec<vec<bool>> vs(r, vec<bool>(c, false));
  for (i64 i = 0; i < r; i++) {
    for (i64 j = 0; j < c; j++) {
      cin >> matrix[i][j];
    }
  ...
  for (i64 i = 0; i < r; i++) {
    for (i64 j = 0; j < c; j++) {
      if (matrix[i][j] == 'L' && vs[i][j] == false) {
        bfs(i, j, r, c, matrix, vs);
        t++;
      }
    }
  }
