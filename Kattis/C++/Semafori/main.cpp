...
int main() {
  ...
  i64 cnt = 0;
  vp64 cpcolors = colors;
  bool verde = false;
  for (i64 i = 0; i < l; i++) {
    for (i64 j = 0; j < colors.size(); j++) {
      colors[j].first--;
      if (colors[j].first == 0) {
        colors[j].second--;
      }
      if (colors[j].second == 0) {
        colors[j].first = cpcolors[j].first;
        colors[j].second = cpcolors[j].second;
      }
    }

    for (i64 x = 0; x < pos.size(); x++) {
      if (i + 1 == pos[x]) {
        if (colors[x].first == 0) {
          cnt++;
          break;
