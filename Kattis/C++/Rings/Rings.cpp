...
bool intersection1(vd64 &x, vd64 &y, vd64 &r, i64 i, i64 j)
{
    d64 dist = sqrt((x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]));
    return (abs(r[i] - r[j]) <= dist && dist <= r[i] + r[j]);
}

void solve(vd64 &x, vd64 &y, vd64 &r, i64 n, vi64 &m)
{
    i64 t = 0;
    i64 max = 0;
    for (i64 i = 0; i < n; i++)
...
int main()
{
    ...
    while (n != -1)
    {
        ...
        if (mx == 1)
        {
            cout << "The largest component contains " << mx << " ring." << endl;
        }
        else 
        {
