...
int main() {
  ...
  vector<set<i64>> v(n + 1);
  vector<i64> sizes(n + 1, 0);
  ...
  while (!q.empty()) {
    ...
    v[crt.first].erase(crt.second);

    if (sizes[crt.first] % 2 == 1) {
      i64 p = sizes[crt.first] / 2 + 1;
      if (p > v[crt.first].size()) {
        for (auto &c : v[crt.first]) {
          if (!visited[c]) {
            q.push({c, crt.first});
          }
        }
