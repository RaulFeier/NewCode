...
deque<p64> fire_points;

void update(vec<vec<char>> &building, i64 lin, i64 col) {
  deque<p64> q;
  q = fire_points;
  fire_points.clear();
  ...
  while (!q.empty()) {
    ...
    for (i64 i : range(4)) {
      i64 xx = x.first + dirx[i];
      i64 yy = x.second + diry[i];

      if (xx >= 0 and xx < lin and yy >= 0 and yy < col and
          building[xx][yy] == '.') {
        building[xx][yy] = '*';
        fire_points.push_back({xx, yy});
      }
    }
  }
}

i64 bfs(vec<vec<char>> &building, p64 start, i64 lin, i64 col, vv &dist) {
  deque<p64> p;
  p.push_back(start);
  vec<vb> visit(lin, vb(col, false));
  ...
  while (!p.empty()) {
    ...
    while (!q.empty()) {
      ...
      if (building[crt.first][crt.second] == '.' and
          (crt.first == 0 or crt.first == lin - 1 or crt.second == col - 1 or
           crt.second == 0)) {
        return dist[crt.first][crt.second];
      }

      for (i64 x : range(4)) {
        i64 xx = crt.first + dirx[x];
        i64 yy = crt.second + diry[x];

        if (xx >= 0 and xx < lin and yy >= 0 and yy < col and
            building[xx][yy] != '#' and building[xx][yy] != '*' and
            !visit[xx][yy]) {
          p.push_back({xx, yy});
...
int main() {
  ...
  while (tc--) {
    ...
    vv dist(lin, vi64(col, 1e9));
    dist[start.first][start.second] = 0;

    i64 res = bfs(building, start, lin, col, dist);
    fire_points.clear();
