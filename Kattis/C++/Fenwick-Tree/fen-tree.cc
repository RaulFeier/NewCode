...
private:
  ...
  inline i64 lsbl(i64 x) { return x & (-x); }

public:
  ftree(i64 n) : ft(n + 1, 0) {}

  inline void update(i64 i, i64 val) {
    i64 crt = i + 1;
    while (crt < ft.size()) {
      ft[crt] += val;
      crt += lsbl(crt);
  ...
  inline i64 sum(i64 from, i64 to) { return sum(to) - sum(from - 1); }
};
