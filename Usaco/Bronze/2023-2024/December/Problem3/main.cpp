  ...
  while (tc--) {
    ...
    for (i64 i = 0; i < n; i++) {
      ...
      for (i64 j = 0; j < n; j++) {
        ...
        if ((height[i] < height[j] and grow[i] <= grow[j])) {
          cnt++;
        } else if (height[i] == height[j] and grow[i] < grow[j]) {
          res = max(res, (height[i] - height[j]) + 1);
          cnt++;
        } else if (height[i] > height[j] and grow[i] < grow[j]) {
          // res = max(res, ((height[i] - height[j]) / (grow[j] - grow[i])) +
          // 1);
          res = func(res, height[i] - height[j], grow[j] - grow[i]);
          cnt++;
        }
      }
