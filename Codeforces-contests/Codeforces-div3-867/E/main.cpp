...
int min_ops_to_anti_palindrome(const string &s) {
  int n = s.length();
  int diff_count = 0;
  for (int i = 0; i < n / 2; i++) {
    if (s[i] != s[n - i - 1]) {
  ...
  if (diff_count == 0) {
    // already an anti-palindrome
    return 0;
  } else if (diff_count == 1 && n % 2 == 1) {
    // odd length string with one different character in the middle
    return 1;
  } else if (diff_count == 1 && n % 2 == 0) {
    // even length string with one different character in the middle
    return 2;
  } else if (diff_count == 2) {
    // swap the two different characters
    return 1;
  } else {
    // anti-palindrome not possible
    return -1;
  }
}
...
int main() {
  ...
  while (tc--) {
    ...
    cout << min_ops_to_anti_palindrome(a) << endl;
  }
