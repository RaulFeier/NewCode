...
i64 solve(vp64 &v, i64 pos, i64 money, i64 x, i64 happy) {
  if (pos == sz(v)) {
    return 0;
  }
  ...
  if (v[pos].first == 0) {
    return dp[pos][happy] =
               solve(v, pos + 1, money + x, x, happy + v[pos].second) +
               v[pos].second;
  }

  if (money < v[pos].first) {
    return dp[pos][happy] = solve(v, pos + 1, money + x, x, happy);
  }

  return dp[pos][happy] = max(solve(v, pos + 1, money + x, x, happy),
                              solve(v, pos + 1, money - v[pos].first + x, x,
                                    happy + v[pos].second) +
                                  v[pos].second);
}
...
int main() {
  ...
  while (tc--) {
    dp.assign(51, vector<i64>(50002, -1));
    i64 m, x;
    cin >> m >> x;
