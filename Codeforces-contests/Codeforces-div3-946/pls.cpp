...
i64 solve(vp64 &v, i64 pos, i64 money, i64 x) {
  if (pos == sz(v)) {
    return 0;
  }

  if (v[pos].first == 0) {
    return solve(v, pos + 1, money + x, x) + v[pos].second;
  }

  if (money < v[pos].first) {
    return solve(v, pos + 1, money + x, x);
  }

  return max(solve(v, pos + 1, money + x, x),
             solve(v, pos + 1, money - v[pos].first + x, x) + v[pos].second);
}
