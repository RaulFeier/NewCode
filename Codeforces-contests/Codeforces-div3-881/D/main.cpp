...
i64 partial_bfs(i64 start, vector<vector<i64>> &graph, vector<bool> vis) {
  deque<i64> q;
  q.push_back(start);
...
i64 bfs(i64 point_one, vector<vector<i64>> &graph) {
  i64 res;
  ...
  while (!q.empty()) {
    ...
    if (node == point_one) {
      res = partial_bfs(node, graph, vis);
    }
