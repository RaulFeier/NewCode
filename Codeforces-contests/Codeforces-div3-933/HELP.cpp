...
vp64 dij(i64 s, const vec<vp64> &g) {
  vp64 ans(g.size() + 1, {INF, INF});
  queue<pair<i64, p64>> pq;
  ...
  while (!pq.empty()) {
    ...
    if (crt.ft > ans[crt.sd.ft].first) {
      continue;
    }

    for (auto &c : g[crt.sd.ft]) {
      if (c.sd == crt.sd.sd) {
        i64 cand = crt.ft;
        if (cand < ans[c.ft].first) {
          ans[c.ft].first = cand;
          ans[c.ft].second = c.sd;
          pq.push({cand, {c.ft, c.sd}});
        } else if (cand == ans[c.ft].first) {
          if (ans[c.ft].second != c.sd) {
            pq.push({cand, {c.ft, c.sd}});
          }
        }
      } else {
