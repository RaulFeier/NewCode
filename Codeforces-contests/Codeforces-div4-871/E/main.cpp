...
i64 bfs(vector<vector<i64>> &eep, vector<vector<bool>> &visit, i64 i, i64 j) {
  deque<p64> q;
  q.push_back({i, j});
  ...
  while (!q.empty()) {
    ...
    visit[crt.first][crt.second] = true;
    res += eep[crt.first][crt.second];
    ...
    for (i64 x : range(4)) {
      ...
      if (xx >= 0 and xx < eep.size() and yy >= 0 and
          yy < eep.front().size() and !visit[xx][yy] and eep[xx][yy] > 0) {
        q.push_back({xx, yy});
        visit[xx][yy] = true;
      }
    }
...
int main() {
  ...
  while (tc--) {
    ...
    vector<vector<i64>> eep(n, vector<i64>(m));
