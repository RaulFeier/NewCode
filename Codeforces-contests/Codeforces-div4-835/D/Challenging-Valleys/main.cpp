...
int main() {
  ...
  while (tc--) {
    ...
    i64 count = 0; // count the valleys

    // case where l = 0 and r != n - 1
    for (i64 i = 0; i < n - 1; i++) {
      if (eep[i + 1] > eep[i]) {
        count++;
        break;
      } else if (eep[i + 1] < eep[i]) {
        break;
      }
    }

    // case where r = n - 1 and l != 0
    ...
    for (i64 i = 0; i < n - 1; i++) {
      ...
      if (!found_valley) {
        if (eep[i] > eep[i + 1]) {
          found_valley = true; // we start couting from left if valley
        }
      }
    }
