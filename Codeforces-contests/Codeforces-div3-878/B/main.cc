...
int count_ways_to_buy_desserts(int n, int k) {
  if (n >= k) {
    // When Toma can buy all desserts
    return k;
  } else {
    // When Toma can buy at most n desserts
    int ways = n + 1;
    int remaining = k - n;

    // Check if remaining desserts can be covered by extra purchases
    if (remaining % n == 0) {
      ways += (remaining / n - 1) * n;
    } else {
      ways += (remaining / n) * n;
    }
...
int main() {
  ...
  while (tc--) {
    ...
    cout << count_ways_to_buy_desserts(n, k) << endl;
  }
